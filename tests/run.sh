...
check "bars stay blue at 83%"          "yes" "$(payload 4 230 83 950 | raw2 | has "${U}█████████"$'\e\\[0m')"
check "bars stay blue at 95%"          "yes" "$(payload 95 42 12 7000 | raw2 | has "${U}██████████"$'\e\\[0m')"
...
check "model purple for Opus too"       "yes" "$(raw2 < "$here/base.json" | has $'\e\\[1;38;5;134mOpus 5')"
check "context word blue"        "yes" "$(row1 < "$here/base.json" | has $'\e\\[38;2;114;124;214mContext\e\\[0m')"
check "model and (1M) purple on Sonnet too" "yes" "$(echo '{"model":{"display_name":"Sonnet 5"},"context_window":{"context_window_size":1000000}}' | raw2 | has $'\e\\[1;38;5;134mSonnet 5\e\\[0m \e\\[38;5;134m(1M)')"
check "XHigh Effort purple, not bold, on Haiku" "yes" "$(echo '{"model":{"display_name":"Haiku 4.5"},"effort":{"level":"xhigh"}}' | raw2 | has $'\e\\[38;5;134mXHigh Effort')"
